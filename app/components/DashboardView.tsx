...
import { ContactLimitNotice, type QuotaUsage } from "./ContactLimitWarning";

/**
 * The dashboard, over data handed to it.
 *
 * This is the whole of what /dashboard used to render inline. It is a
 * component over props now because an admin needs to open any user's dashboard
 * as that user sees it (/admin/users/[userId]), and "as that user sees it" is
 * only true while there is one implementation. A second copy built for admins
 * would start identical and drift within a release.
 *
 * The two callers differ only in where the data comes from and in readOnly:
 *
 *  - /dashboard passes the current user's queries and leaves readOnly off.
 *  - /admin/users/[userId] passes the admin-scoped queries for that user and
 *    sets readOnly, which drops every control that would act on, or navigate
 *    into, the reader's own account. Nothing here mutates in either mode; the
 *    flag exists because the links resolve to whoever is signed in, so on
 *    someone else's dashboard they would lie about where they go.
 */

export type DashboardEmailStats = FunctionReturnType<typeof api.emailStats.getForCurrentUser>;

export type DashboardViewProps = {
  /** undefined while loading. */
  domains: Doc<"domains">[] | undefined;
  /** undefined while loading, null when there is no user to report on. */
  emailStats: DashboardEmailStats | undefined;
  /** Feeds the contact allowance banner. undefined while loading. */
  usage: QuotaUsage | undefined;
  /** True when an admin is reading someone else's dashboard. */
  readOnly?: boolean;
  /** Rendered above the dashboard header, for the admin banner. */
  banner?: ReactNode;
};
...
export default function DashboardView({
  ...
  const domainsCardClass =
    "rounded-xl border border-gray-200 bg-white p-6 transition-shadow dark:border-gray-700 dark:bg-gray-800";
  ...
  return (
    <div className="p-8">
      ...
      <ContactLimitNotice usage={usage} readOnly={readOnly} />

      {/* Stats grid */}
      <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        {/* Old: always a Link to /domains. That route shows whoever is signed
            in, so on an admin's read only view of another account it would
            have taken the admin to their own domains. */}
        {readOnly ? (
          <div className={domainsCardClass}>
          ...
          <Link href="/domains" className={`${domainsCardClass} hover:shadow-md`}>
            <p className="text-sm font-medium text-gray-500 dark:text-gray-400">Domains</p>
            <p className="mt-2 text-3xl font-bold text-gray-900 dark:text-white">
              {isLoading ? (
                <span className="inline-block h-8 w-8 animate-pulse rounded bg-gray-200 dark:bg-gray-700" />
              ...
              <ResponsiveContainer width="100%" height={280}>
                <PieChart>
                  <Pie
                    ...
                    paddingAngle={3}
                    dataKey="value"
                    // Old: (props: any), which tripped no-explicit-any. Recharts
                    // types this callback as taking any, so a narrower parameter
                    // is still assignable and names the two fields it reads.
      ...
      {/* Quick actions. Hidden in read only mode: Add Domain writes a domain,
          and it would be written against the admin's own account. */}
      {!readOnly && (
        ...
        {isLoading ? (
          ...
          <div className="divide-y divide-gray-50 dark:divide-gray-700/50">
            {domains.map((domain: Doc<"domains">) => {
              ...
              // The domain detail route checks ownership, so in read only mode
              // the link would land an admin on a page that refuses to load.
              // The admin domain tools at /admin/domains are the way in.
              if (readOnly) {
                return (
                  <div key={domain._id} className="flex items-center justify-between px-6 py-4">
                    {row}
                  </div>
                );
              }
