  ...
  return (
    <header className="sticky top-0 z-50 border-b border-gray-100 bg-white/80 backdrop-blur-md">
      <div className="mx-auto flex max-w-7xl items-center justify-between px-6 py-4">
        ...
        <nav className="hidden items-center gap-8 md:flex">
          ...
          <Unauthenticated>
            <SignInButton mode="modal" forceRedirectUrl="/dashboard">
              <button className="text-sm font-medium text-gray-600 transition-colors hover:text-violet-600">
                Sign In
              </button>
            </SignInButton>
        ...
        >
          <span
            className={`h-0.5 w-6 bg-gray-800 transition-transform ${mobileOpen ? "translate-y-2 rotate-45" : ""}`}
          />
          <span
            className={`h-0.5 w-6 bg-gray-800 transition-opacity ${mobileOpen ? "opacity-0" : ""}`}
          />
          <span
            className={`h-0.5 w-6 bg-gray-800 transition-transform ${mobileOpen ? "-translate-y-2 -rotate-45" : ""}`}
          />
