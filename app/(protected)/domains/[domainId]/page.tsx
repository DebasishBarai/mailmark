...
const handleRemoveDomain = async () => {
  ...
  function generateZoneFile(): string {
    const lines: string[] = [
      `; Mailmark DNS records for ${domain!.domain}`,
    ...
    for (const record of dnsRecords) {
      ...
      if (record.type === "CNAME") {
        ...
        lines.push(`${name}\tIN\tMX\t${"priority" in record ? record.priority : "10"}\t${record.value}.`);
      } else if (record.type === "TXT") {
        lines.push(`${name}\tIN\tTXT\t"${record.value}"`);
      }
    }
  ...
  function handleCopyZoneFile() {
    navigator.clipboard.writeText(generateZoneFile());
    setZoneCopied(true);
    setTimeout(() => setZoneCopied(false), 2000);
  }

  function handleDownloadZoneFile() {
    const blob = new Blob([generateZoneFile()], { type: "text/plain" });
  ...
  return (
    <div className="p-4 pb-6 md:p-8">
          ...
          {dkimTokens.length > 0 && (
            <div className="flex shrink-0 items-center gap-2">
              <button
                ...
                className="inline-flex items-center gap-1.5 rounded-lg border border-gray-200 px-2.5 py-1.5 text-xs font-medium text-gray-600 transition-colors hover:bg-gray-50 dark:border-gray-600 dark:text-gray-400 dark:hover:bg-gray-700"
                title="Copy zone file to clipboard"
              >
                {zoneCopied ? (
                  <>
                  ...
                  <div className="mb-2 flex items-center justify-between">
                    <div className="flex items-center gap-2">
                      ...
                      <svg
                        ...
                        strokeWidth={1.5}
                        stroke="currentColor"
                        data-tooltip-id="dns-tooltip"
                        data-tooltip-content={record.tooltip}
                      >
      ...
      <Tooltip id="dns-tooltip" className="!max-w-[220px] !text-xs !leading-relaxed !z-50" />
    </div>
  );
}
