...
/**
 * The page used to call every query at its own top level:
 *
 *   const domains = useQuery(api.domains.listForCurrentUser);
 *   const emailStats = useQuery(api.emailStats.getForCurrentUser);
 *
 * and hand the results down. Convex rethrows a backend error from useQuery
 * during render, so with the queries up here a single failing one threw from
 * DashboardPage itself. Nothing in the app caught it, and Next.js answered by
 * replacing the whole document with "Application error: a client-side
 * exception has occurred" - which is what a customer reported after
 * emailStats.getForCurrentUser went over the Convex read limit on a large
 * account. The stat cards, the charts, the domain list and the contact notice
 * were all fine, and none of them rendered.
 *
 * Each section below now owns the query it needs and sits behind its own
 * boundary, so a query that fails costs one panel rather than the page. The
 * two sections that read domains subscribe to the same query, which Convex
 * dedupes, so this costs no extra round trip.
 */
    ...
    >
      ...
      <p className="mt-2 text-3xl font-bold text-gray-900 dark:text-white">
        {isLoading ? (
          ...
          <RollingCounter value={domains?.length ?? 0} />
        )}
      </p>
...
function EmailCharts() {
  ...
  if (!emailStats || emailStats.totalSent + emailStats.totalInbox <= 0) return null;
...
function DomainsList({ onAddDomain }: { onAddDomain: () => void }) {
  const domains = useQuery(api.domains.listForCurrentUser);
  const isLoading = domains === undefined;
...
export default function DashboardPage() {
  ...
  return (
    <div className="p-8">
      ...
      <SectionErrorBoundary label="The contact allowance notice" fallbackClassName="mb-8">
        <ContactLimitWarning />
      </SectionErrorBoundary>

      {/* Stats grid. Each boundary renders its children directly when nothing
          has failed, so the cards stay direct children of the grid and keep
          their columns. Only a fallback introduces a wrapper, which is why the
          three-card section sizes its own. */}
      <div className="mb-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">
        <SectionErrorBoundary label="The domain count">
          <DomainCountCard />
        </SectionErrorBoundary>
        <SectionErrorBoundary
          label="Your email stats"
          fallbackClassName="sm:col-span-2 lg:col-span-3"
        >
          <EmailStatCards />
        </SectionErrorBoundary>
      </div>
      ...
      {/* Charts */}
      <SectionErrorBoundary label="The email volume charts" fallbackClassName="mb-8">
        <EmailCharts />
      </SectionErrorBoundary>
      ...
      {/* Domains list */}
      <SectionErrorBoundary label="Your domain list">
        <DomainsList onAddDomain={() => setShowAddModal(true)} />
      </SectionErrorBoundary>
