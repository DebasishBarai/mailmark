...
/**
 * Every number the dashboard draws, for one user.
 *
 * This was the body of getForCurrentUser. It is a plain function over a userId
 * now so that the admin view at /admin/users/[userId] renders from the same
 * arithmetic the user's own dashboard renders from. Two copies of this would
 * drift, and an admin reading a number the user cannot see is worse than no
 * admin view at all.
 */
async function statsForUser(ctx: QueryCtx, userId: Id<"users">) {
...
export const getForCurrentUser = query({
  ...
  handler: async (ctx) => {
    ...
    return await statsForUser(ctx, user._id);
  },
});

/**
 * The same stats for any user, for an admin. Read only.
 *
 * Returns null for a non-admin or a missing user rather than throwing: the
 * page that reads this takes the id off the URL, where a thrown query would
 * crash the route instead of showing an empty state.
 */
export const getForUserAsAdmin = query({
